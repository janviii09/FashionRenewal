...
export class TrustedLenderService {
    ...
    async calculateEligibility(userId: number): Promise<boolean> {
        ...
        const lastDisputeDate = user.disputesAgainst.length > 0
        ...
        const metrics = await this.prisma.trustedLenderMetrics.upsert({
            ...
            update: {
                ...
                currentTrustScore: user.trustScore,
                isEligible: eligible,
                badgeGrantedAt: eligible && !user.trustedLenderMetrics?.badgeGrantedAt
                    ? new Date()
                    : user.trustedLenderMetrics?.badgeGrantedAt,
                badgeRevokedAt: !eligible && user.trustedLenderMetrics?.isEligible
                    ? new Date()
    ...
    async revokeBadge(userId: number, reason: string): Promise<void> {
        ...
        this.logger.warn(`Trusted lender badge revoked for user ${userId}: ${reason}`);
    }
    ...
    async recalculateAllBadges(): Promise<void> {
        ...
        const metricsToRecalculate = await this.prisma.trustedLenderMetrics.findMany({
            where: {
        ...
        this.logger.log(`Trusted lender badge recalculation complete. Updated: ${updated} users`);
    }
