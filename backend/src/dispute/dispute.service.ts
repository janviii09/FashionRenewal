...
export class DisputeService {
    ...
    async createDispute(
        ...
        const existingDispute = await this.prisma.dispute.findFirst({
            where: {
                orderId: dto.orderId,
                status: {
                    in: [DisputeStatus.OPEN, DisputeStatus.RESPONDED, DisputeStatus.UNDER_REVIEW, DisputeStatus.ESCALATED],
                },
            },
        });
    ...
    async respondToDispute(
        ...
        if (dispute.status !== DisputeStatus.OPEN) {
            throw new BadRequestException('This dispute has already been responded to');
        }
    ...
    async resolveDispute(
        ...
        const trustScoreImpact = this.calculateTrustScoreImpact(dispute.reason, dto.resolution);
    ...
    async getDisputeById(disputeId: number): Promise<Dispute> {
        const dispute = await this.prisma.dispute.findUnique({
            ...
            include: {
                ...
                against: { select: { id: true, name: true, email: true, trustScore: true } },
                resolvedBy: { select: { id: true, name: true } },
                order: {
                    include: {
