...
export class ValidationService {
    ...
    async getPendingValidations(userId: number) {
        ...
        if (!user || (user.role !== Role.VALIDATOR && user.role !== Role.ADMIN && user.role !== Role.SUPER_ADMIN)) {
            throw new ForbiddenException('Only validators can view pending validations');
        }
     ...
     */
    async approveValidation(validationId: number, validatorId: number, notes?: string) {
        const validator = await this.prisma.user.findUnique({
            where: { id: validatorId },
        });

        if (!validator || (validator.role !== Role.VALIDATOR && validator.role !== Role.ADMIN && validator.role !== Role.SUPER_ADMIN)) {
     ...
     */
    async rejectValidation(validationId: number, validatorId: number, notes: string) {
        const validator = await this.prisma.user.findUnique({
            where: { id: validatorId },
        });
