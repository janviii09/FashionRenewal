...
export class RecommendationService {
  ...
  async getPersonalizedRecommendations(
    ...
    try {
      ...
      if (!userId) {
        return this.getPopularItems(limit, userId);
      }
      ...
      const candidates = await this.prisma.wardrobeItem.findMany({
        where: {
          ...
          // STRICT EXCLUSION: Own items & purchases
  ...
  private calculateHybridScore(
    ...
    const itemSimilarityScore = this.getItemSimilarityScore(
      item,
      recentActivities,
    );
    ...
    const totalScore =
      itemSimilarityScore * 0.3 +
      userBehaviorScore * 0.4 +
   ...
   */
  private getItemSimilarityScore(
    item: WardrobeItem,
    activities: any[],
  ): number {
    let maxSimilarity = 0;
  ...
  private getUserBehaviorScore(item: WardrobeItem, activities: any[]): number {
    ...
    const categoryActivities = activities.filter(
      (a) => a.category === item.category || a.item?.category === item.category,
    );
  ...
  private generateReason(
    ...
    const recentPurchases = activities.filter(
      (a) =>
        (a.eventType === "PURCHASE_COMPLETE" ||
          a.eventType === "RENTAL_COMPLETE") &&
    ...
    const categoryScores =
      (affinity?.categoryScores as Record<string, number>) || {};
    ...
    const wishlistItems = activities.filter(
      (a) =>
  ...
  private async getPopularItems(
    ...
    if (userId) {
      Object.assign(query, getMarketplaceExclusionFilter(userId));
    }
      ...
      const candidates = await this.prisma.wardrobeItem.findMany({
        where: {
          ...
          ...(sourceProduct.category && {
            OR: [{ category: sourceProduct.category }],
          }),
          // Apply exclusion if user logged in
          ...(userId ? getMarketplaceExclusionFilter(userId) : {}),
        },
        take: 500,
      });
  ...
  async getRecommendationsWithMetadata(
    productId: number,
    limit: number = 12,
    userId?: number,
  ) {
