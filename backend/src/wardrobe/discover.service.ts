...
export class DiscoverService {
  ...
  async discover(query: DiscoverQuery): Promise<DiscoverResponse> {
    ...
    try {
      ...
      Object.assign(where, getMarketplaceExclusionFilter(query.userId));
  ...
  private buildWhereClause(
    ...
    if (query.availability) {
      // Validate that it's a valid ItemAvailability enum value
      if (
        Object.values(ItemAvailability).includes(
          query.availability as ItemAvailability,
        )
      ) {
        where.availability = query.availability as ItemAvailability;
      }
    }
    ...
    // Condition filter
    if (query.condition) {
      // Validate that it's a valid ItemCondition enum value
      if (
        Object.values(ItemCondition).includes(query.condition as ItemCondition)
      ) {
        where.condition = query.condition as ItemCondition;
      }
    }
  ...
  private buildOrderBy(
    ...
    switch (sort) {
      ...
      case "trending":
        return [{ trendingScore: "desc" }, { createdAt: "desc" }];

      case "most-rented":
        return [{ rentalCount: "desc" }, { viewCount: "desc" }];

      case "price-asc":
        return [{ rentPricePerDay: "asc" }, { sellPrice: "asc" }];

      case "price-desc":
        return [{ rentPricePerDay: "desc" }, { sellPrice: "desc" }];
