...
export class BrowseService {
     ...
     */
    async getBrowsePreview(): Promise<BrowsePreviewResponse> {
        const startTime = Date.now();

        try {
            // Fetch all sections in parallel for performance
            const [featured, trending, mostRented, newlyAdded, total] = await Promise.all([
                this.getFeaturedItems(4),
                this.getTrendingItems(4),
                this.getMostRentedItems(4),
                this.getNewlyAddedItems(4),
                this.getTotalMarketplaceCount()
            ]);
     ...
     */
    private async getFeaturedItems(limit: number): Promise<WardrobeItem[]> {
     ...
     */
    private async getTrendingItems(limit: number): Promise<WardrobeItem[]> {
        return this.prisma.wardrobeItem.findMany({
            where: {
                availability: {
                    in: ['AVAILABLE_FOR_RENT', 'AVAILABLE_FOR_SALE', 'AVAILABLE_FOR_SWAP']
     ...
     */
    private async getMostRentedItems(limit: number): Promise<WardrobeItem[]> {
        return this.prisma.wardrobeItem.findMany({
            where: {
                availability: {
                    in: ['AVAILABLE_FOR_RENT', 'AVAILABLE_FOR_SALE', 'AVAILABLE_FOR_SWAP']
     ...
     */
    private async getNewlyAddedItems(limit: number): Promise<WardrobeItem[]> {
        return this.prisma.wardrobeItem.findMany({
            where: {
                availability: {
                    in: ['AVAILABLE_FOR_RENT', 'AVAILABLE_FOR_SALE', 'AVAILABLE_FOR_SWAP']
     ...
     */
    private async getTotalMarketplaceCount(): Promise<number> {
